...
class subs:
    ...
    def get_subs_v3(content_urls: list, output_path="sub_merge", should_cleanup=True, specific_files_cleanup=None):
        ...
        for index, url_container in enumerate(content_urls):
            ...
            for each_url in url_container.get("url", []):
                ...
                try:
                    ...
                    extracted = []
                    # 1. Direct Regex (handles Markdown and Plain Text natively)
                    ...
                    # 3. Fallback to Subconverter for YAML/Clash configs
                    if not extracted:
            ...
            # Write to sub/list/xx.txt for README node count per repository
            ...
            # Add to the global pool for this chunk
        ...
        # Deduplicate across the whole chunk
        unique_links = list(set([p['link'] for p in all_clash_proxies]))
        os.makedirs(sub_merge_path, exist_ok=True)
        
        # YAML representation (minimal format just for GitHub Action string counting)
        ...
        # Base64 representation (what the tester actually reads)
